...
namespace progbox {
    ...
    public:
        ...
        void export_input_csv(const std::filesystem::path& dir) const {
            ...
            // ── Header ──
            f << "PlayerID,Name,Team,Age,PER,DWS,EWA,BaselineOvr";
        ...
        void export_raw_csv(const std::filesystem::path& dir) const {
            std::filesystem::create_directories(dir);
            std::ofstream f(dir / "outputs.csv");
            if (!f.is_open()) return;
            ...
            // ── Header ──
            f << "Run,RunSeed,Name,Team,Age,PlayerID,Baseline,Ovr,Delta,PctChange,AboveBaseline,PER,DWS,EWA";
            for (const char* attr : ALL_ATTRS) {
                f << std::format(",{}", attr);
            }
            write_stats_header(f);
        ...
        void export_summary_csv(const std::filesystem::path& dir) const {
            std::filesystem::create_directories(dir);
            std::ofstream f(dir / "summary.csv");
            if (!f.is_open()) return;
            ...
            for (size_t p = 0; p < n_players_; ++p) {
                ...
                double mean_delta = std::accumulate(deltas.begin(), deltas.end(), 0.0) / static_cast<double>(n_runs_);
                ...
                long count_pos = std::count_if(deltas.begin(), deltas.end(), [](double d){ return d > 0; });
        ...
        void export_godprogs(const std::filesystem::path& dir) const {
            ...
            if (total_events > 0) {
                ...
                std::printf(" God-progs: no events found\n");
            }
