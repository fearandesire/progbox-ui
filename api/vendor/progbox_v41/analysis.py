...
def _load(raw_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    df = pd.read_csv(raw_dir / 'outputs.csv', index_col=0)
...
def _chart4(ppl: pd.DataFrame, out: Path) -> None:
    ...
    Outliers (top 88th pct by |delta| or σ) labelled with display labels.
    """
    ...
    mask = ((ppl['StdDelta']          > ppl['StdDelta'].quantile(0.88)) |
            (ppl['MeanDelta'].abs()   > ppl['MeanDelta'].abs().quantile(0.88)))
    for _, row in ppl[mask].iterrows():
        ax.annotate(row['Label'], (row['MeanDelta'], row['StdDelta']),
                    xytext=(5, 4), textcoords='offset points',
                    fontsize=6.5, color=_PAL['subtle'], alpha=0.9)
