 ...
 * cross-request collisions that recursive mkdir would silently share.
 */
async function allocateBuildDir(
  startFrom?: string,
): Promise<{ build: string; out: string }> {
  ...
  throw new Error("Unable to allocate a unique build directory");
}
